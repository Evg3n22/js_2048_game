...
const game = new (0, _gameClassJsDefault.default)(valueMatrix);
...
document.addEventListener("keydown", (e)=>{
    if (btnClicked) {
        ...
        if (currentStatus === "win") messageWin.classList.remove("hidden");
        if (currentStatus === "lose") messageLose.classList.remove("hidden");
    }
});
...
class Game {
   ...
   */ constructor(initialState){
        ...
        this.size = 4;
        if (initialState === undefined) this.state = [
            [
                0,
                0,
                0,
    ...
    moveLeft() {
        ...
        this.state.forEach((row, rowIndex)=>{
            row.forEach((cell, cellIndex)=>{
                if (this.state[rowIndex][cellIndex] === this.state[rowIndex][cellIndex + 1]) {
        ...
        this.state.forEach((row, rowIndex)=>{
            while(row.length < 4)this.state[rowIndex].push(0);
            if (!this.arraysEqual(row, oldMatrix[rowIndex])) moved = true;
        });
        if (moved) this.generateValue(1);
    ...
    moveRight() {
        ...
        // fill empty cells with 0
        this.state.forEach((row, rowIndex)=>{
            while(row.length < 4)this.state[rowIndex].unshift(0);
            if (!this.arraysEqual(row, oldMatrix[rowIndex])) moved = true;
        });
        if (moved) this.generateValue(1);
        this.score = this.getScore();
    ...
    moveUp() {
        ...
        this.state.forEach((row, rowIndex)=>{
            ...
            if (!this.arraysEqual(col, oldCol)) moved = true;
        });
        if (moved) this.generateValue(1);
        this.score = this.getScore();
    }
   ...
   */ getScore() {
        ...
        Array.from(this.state).forEach((row)=>{
            row.forEach((cell)=>{
                if (/^\d+$/.test(cell)) score += Number(cell);
            });
        });
        return score;
    }
   ...
   */ getStatus() {
        for(let rowIndex = 0; rowIndex < this.state.length; rowIndex++)for(let cellIndex = 0; cellIndex < this.state[rowIndex].length; cellIndex++){
            ...
            if (cellIndex + 1 < 4 && this.state[rowIndex][cellIndex] === this.state[rowIndex][cellIndex + 1]) {
                this.status = "playing";
                return this.status;
            }
            if (rowIndex + 1 < 4 && this.state[rowIndex][cellIndex] === this.state[rowIndex + 1][cellIndex]) {
                this.status = "playing";
                return this.status;
            }
        }
   ...
   */ restart() {
    ...
    getEmtyCells() {
        ...
        Array.from(this.state).forEach((row, rowIndex)=>{
            row.forEach((cell, cellIndex)=>{
                if (this.state[rowIndex][cellIndex] === 0) available.push([
                    rowIndex,
                    cellIndex
                ]);
            });
